...
class FaceFusionClient:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/api/queue/submit", json=job_data
                ) as response:
    ...
    async def poll_job(
        self, job_id: str, poll_interval: int = 2, timeout: int = 300
    ) -> Optional[Dict[str, Any]]:
...
api_router = APIRouter(
    deprecated=True, tags=["⚠️ DEPRECATED - Legacy API (Use /api/v1 instead)"]
)
...
class ModelConfigurationPreset(BaseModel):
    ...
    base_model: Optional[str] = (
        None  # Base model type (e.g., "sdxl", "flux_dev", "pony")
    )
    is_global: Optional[bool] = (
        False  # If true, preset applies to all models of this base_model type
    )
...
class Scene(BaseModel):
    ...
    parent_scene_id: Optional[str] = (
        None  # Reference to original scene if this is an alternate
    )
...
class GeneratedContent(BaseModel):
    ...
    model_type: Optional[str] = (
        None  # "sdxl", "flux_dev", "flux_krea", "wan_2_1", "wan_2_2", "hidream"
    )
...
class Clip(BaseModel):
    ...
    parent_clip_id: Optional[str] = (
        None  # Reference to original clip if this is an alternate
    )
    alternate_number: int = 0  # 0 = original, 1 = A1, 2 = A2, etc.
    is_alternate: bool = False  # Quick flag to identify alternates
...
class Character(BaseModel):
    ...
    generation_method: Optional[str] = (
        "ip_adapter"  # ip_adapter, reactor, instantid, lora
    )
...
class GenerationPool(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = (
        None  # Additional info (resolution, duration, etc.)
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
def get_model_defaults(model_name: str, preset: str = "fast") -> Dict[str, Any]:
    ...
    return model_config.get(
        preset, model_config.get("fast", model_config.get("quality", {}))
    )
...
class ComfyUIClient:
    ...
    async def _check_runpod_connection(self) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if data.get("status") in ["RUNNING", "READY", "IDLE"]:
                            logger.info(
                                f"RunPod endpoint {self.endpoint_id} is {data.get('status')}"
                            )
                            return True
                        else:
                            logger.warning(
                                f"RunPod endpoint {self.endpoint_id} status: {data.get('status')}"
                            )
                            return False
                    elif response.status == 401:
                        logger.error(
                            f"Invalid API key for RunPod endpoint {self.endpoint_id}"
                        )
                        return False
            ...
            try:
                ...
                async with session.get(
                    info_url, headers=headers, timeout=5
                ) as response:
                    if response.status == 200:
                        ...
                        if "input" in node_info and "required" in node_info["input"]:
                            ...
                            for input_name, input_info in required_inputs.items():
                                if isinstance(input_info, list) and len(input_info) > 0:
                                    if (
                                        "ckpt_name" in input_name.lower()
                                        or "checkpoint" in input_name.lower()
                                    ):
    ...
    async def _get_runpod_models(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "loras": ["lcm-lora-sdxl.safetensors", "pytorch_lora_weights.safetensors"],
            "vaes": ["sdxl_vae.safetensors", "vae-ft-mse-840000-ema-pruned.ckpt"],
        }
                            ...
                            async with session.get(
                                ...
                                if status_response.status == 200:
                                    ...
                                    if status == "COMPLETED":
                                        ...
                                        logging.error(
                                            f"RunPod generation failed: {error_msg}"
                                        )
                                        break
    ...
    async def _generate_video_runpod(
        ...
        runpod_input = {"prompt": prompt, "generation_type": "video"}
                            ...
                            async with session.get(
                                ...
                                if status_response.status == 200:
                                    ...
                                    if status == "COMPLETED":
                                        ...
                                        logging.error(
                                            f"RunPod video generation failed: {error_msg}"
                                        )
                                        break
                                        ...
                                        if prompt_id in history:
                                            ...
                                            for node_id, output in outputs.items():
                                                if (
                                                    ...
                                                    video_files = output.get(
                                                        "gifs", output.get("videos", [])
                                                    )
                                                    if video_files:
    ...
    async def _create_wan_video_workflow(
        self, prompt: str, negative_prompt: str, model: str, params: Dict, loras: List
    ) -> Dict:
    ...
    async def generate_character_with_ip_adapter(
        self, prompt: str, character_image: str, model: str = None, params: Dict = None
    ) -> Optional[str]:
        """Generate consistent character using IP-Adapter"""
        try:
            if self.server_type == "runpod":
                return await self._generate_ip_adapter_runpod(
                    prompt, character_image, model, params
                )
            else:
                return await self._generate_ip_adapter_standard(
                    prompt, character_image, model, params
                )
        except Exception as e:
    ...
    async def generate_character_with_reactor(
        self,
        prompt: str,
        face_image: str,
        ...
        try:
            if self.server_type == "runpod":
                return await self._generate_reactor_runpod(
                    prompt, face_image, target_image, model, params
                )
            else:
                return await self._generate_reactor_standard(
                    prompt, face_image, target_image, model, params
                )
        except Exception as e:
    ...
    async def generate_character_with_instantid(
        self,
        prompt: str,
        face_image: str,
        pose_image: str = None,
        ...
        try:
            if self.server_type == "runpod":
                return await self._generate_instantid_runpod(
                    prompt, face_image, pose_image, model, params
                )
            else:
                return await self._generate_instantid_standard(
                    prompt, face_image, pose_image, model, params
                )
        except Exception as e:
    ...
    async def _generate_ip_adapter_standard(
        self, prompt: str, character_image: str, model: str = None, params: Dict = None
    ) -> Optional[str]:
        ...
        workflow = {
            ...
            "4": {
                "inputs": {"image": character_image, "model": ["1", 0]},
            ...
            "6": {
                "inputs": {"ckpt_name": "clip_vision.safetensors"},
    ...
    async def _generate_reactor_standard(
        self,
        prompt: str,
        face_image: str,
        target_image: str = None,
        ...
        workflow = {
            ...
            "4": {"inputs": {"image": face_image}, "class_type": "LoadImageMask"},
            "5": {
                "inputs": {
            ...
            "8": {
                "inputs": {"filename_prefix": "character_reactor", "images": ["7", 0]},
                "class_type": "SaveImage",
            },
        }
    ...
    async def _generate_ip_adapter_runpod(
        self, prompt: str, character_image: str, model: str = None, params: Dict = None
    ) -> Optional[str]:
        """IP-Adapter for RunPod (simplified)"""
        # For now, fall back to standard generation
        return await self.generate_image(
            prompt, "low quality, blurry", model, params, []
        )

    async def _generate_reactor_runpod(
        self,
        prompt: str,
        face_image: str,
        target_image: str = None,
    ...
    async def _generate_instantid_runpod(
        self,
        prompt: str,
        face_image: str,
        pose_image: str = None,
    ...
    async def _generate_instantid_standard(
        self,
        prompt: str,
        face_image: str,
        pose_image: str = None,
        ...
        return await self._generate_ip_adapter_standard(
            prompt, face_image, model, params
        )

    async def generate_with_openpose(
        self,
        prompt: str,
        reference_image: str,
        negative_prompt: str = "",
        ...
        try:
            if self.server_type == "runpod":
                return await self._generate_openpose_runpod(
                    prompt, reference_image, negative_prompt, model, params
                )
            else:
                return await self._generate_openpose_standard(
                    prompt, reference_image, negative_prompt, model, params
                )
        except Exception as e:
    ...
    async def generate_with_depth(
        ...
        try:
            if self.server_type == "runpod":
                return await self._generate_depth_runpod(
                    prompt, reference_image, negative_prompt, model, params
                )
            else:
                return await self._generate_depth_standard(
                    prompt, reference_image, negative_prompt, model, params
                )
        except Exception as e:
    ...
    async def _generate_openpose_standard(
        self,
        prompt: str,
        reference_image: str,
        negative_prompt: str = "",
        ...
        workflow = {
            ...
            "3": {
                "inputs": {
                    ...
                    or "low quality, blurry, bad anatomy, distorted",
                    "clip": ["1", 1],
                },
                "class_type": "CLIPTextEncode",
            },
            "4": {"inputs": {"image": reference_image}, "class_type": "LoadImage"},
            "5": {
                "inputs": {
            ...
            "10": {
                "inputs": {"filename_prefix": "character_openpose", "images": ["9", 0]},
                "class_type": "SaveImage",
            },
        }
    ...
    async def _generate_depth_standard(
        self,
        prompt: str,
        reference_image: str,
        negative_prompt: str = "",
        ...
        workflow = {
            ...
            "10": {
                "inputs": {"filename_prefix": "character_depth", "images": ["9", 0]},
                "class_type": "SaveImage",
            },
        }
    ...
    async def _generate_openpose_runpod(
        self,
        prompt: str,
        reference_image: str,
        negative_prompt: str = "",
        ...
        return await self.generate_image(
            prompt, negative_prompt or "low quality, blurry", model, params, []
        )

    async def _generate_depth_runpod(
        self,
        prompt: str,
        reference_image: str,
        negative_prompt: str = "",
...
async def root():
    ...
    warnings.warn(
        "Legacy /api/ endpoint is deprecated. Use /api/v1/", DeprecationWarning
    )
...
async def health_check():
    ...
    warnings.warn(
        "Legacy /api/health endpoint is deprecated. Use /api/v1/health",
        DeprecationWarning,
    )
    logging.warning(
        "Legacy /api/health endpoint accessed. Client should migrate to /api/v1/health"
    )
    health_status = {
        "status": "healthy",
...
async def add_comfyui_server(
    server_data: ComfyUIServerCreate, db_conn=Depends(get_database)
):
...
async def get_server_info(server_id: str, db_conn=Depends(get_database)):
    ...
    models_data = (
        ...
        if not is_online
        else await client.get_models()
    )

    models = [
        Model(name=name, type="checkpoint")
    ...
    return ComfyUIServerInfo(
        server=server, models=models, loras=loras, is_online=is_online
    )
...
async def update_model(
    model_id: str, model_update: DatabaseModelUpdate, db_conn=Depends(get_database)
):
    """Update a model"""
...
async def sync_model_civitai(model_id: str, db_conn=Depends(get_database)):
    ...
    if active_models_service:
        active_model = await active_models_service.get_active_models(
            backend_id=None, model_type=None
        )
    ...
    try:
        ...
        if not best_match:
            ...
            fallback_match = await find_best_civitai_match_mongodb(
                model_data["name"], db_conn, fallback=True
            )
            if fallback_match:
                ...
                raise HTTPException(
                    status_code=404,
                    detail="No matching model found in Civitai database",
                )
        else:
        ...
        first_version = (
            best_match.get("modelVersions", [{}])[0]
            if best_match.get("modelVersions")
            else {}
        )
        ...
        civitai_info = CivitaiModelInfo(
            ...
            downloadUrl=first_version.get("files", [{}])[0].get("downloadUrl")
            if first_version.get("files")
            else None,
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync from Civitai database: {str(e)}"
        )


@api_router.post("/models/{model_id}/search-civitai")
async def search_civitai_matches(
    model_id: str, request: CivitaiSearchRequest, db_conn=Depends(get_database)
):
    ...
    try:
        ...
        exact_match = await db_conn.database_models.find_one(
            {"source": "civitai_sdxl", "name": {"$regex": query, "$options": "i"}}
        )
        ...
        for doc in text_search_results:
            ...
            if any(
                m["civitai_model"]["modelId"] == doc["civitai_info"]["modelId"]
                for m in matches
            ):
                continue
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to search Civitai database: {str(e)}"
        )
...
async def get_active_models(
    ...
    if not active_models_service:
        raise HTTPException(
            status_code=503, detail="Active models service not available"
        )
    ...
    models = await active_models_service.get_active_models(
        backend_id=backend_id, model_type=type_enum
    )
...
async def link_civitai_model(
    model_id: str, request: CivitaiLinkRequest, db_conn=Depends(get_database)
):
    ...
    try:
        ...
        if not civitai_api_key:
            raise HTTPException(
                status_code=400, detail="Civitai API key not configured"
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(model_url, headers=headers) as response:
                if response.status != 200:
                    raise HTTPException(
                        status_code=400, detail="Failed to fetch Civitai model"
                    )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch from Civitai: {str(e)}"
        )
...
async def create_model_preset(
    model_id: str, preset_data: ModelConfigurationPreset, db_conn=Depends(get_database)
):
...
@api_router.post(
    "/models/presets/global/{base_model}", response_model=ModelConfigurationPreset
)
...
@api_router.get(
    "/models/presets/global/{base_model}", response_model=List[ModelConfigurationPreset]
)
async def get_global_presets(base_model: str, db_conn=Depends(get_database)):
    ...
    global_presets = await db_conn.model_presets.find(
        {"base_model": base_model, "is_global": True}
    ).to_list(100)
...
@api_router.put(
    "/models/{model_id}/presets/{preset_id}", response_model=ModelConfigurationPreset
)
async def update_model_preset(
    ...
    await db_conn.model_presets.update_one(
        {"id": preset_id}, {"$set": preset_update.dict(exclude_unset=True)}
    )
...
async def delete_model_preset(
    model_id: str, preset_id: str, db_conn=Depends(get_database)
):
...
async def sync_single_model(name: str, type: str, server_id: str, db_conn):
    ...
    existing = await db_conn.database_models.find_one({"name": name, "type": type})
...
async def find_best_civitai_match_db(
    server_model_name: str, db_conn, fallback: bool = False
) -> dict:
    """
    ...
    def clean_model_name(name: str) -> str:
        ...
        name = re.sub(
            r"^(sd|stable-diffusion|sd-|sdxl|sd_xl)_?", "", name, flags=re.IGNORECASE
        )
        name = re.sub(
            r"_v\d+(-\d+)?(_lightning)?(_bakedvae)?", "", name, flags=re.IGNORECASE
        )
    ...
    exact_match = await db_conn.civitai_models.find_one({"name": server_name_original})
    if exact_match:
    ...
    text_search = (
        await db_conn.civitai_models.find(
            {"$text": {"$search": server_name_clean}}, {"score": {"$meta": "textScore"}}
        )
...
async def get_model_profiles(civitai_model: dict, db_conn) -> list:
    ...
    for profile in profiles:
        print(
            f"  - {profile['name']} ({profile['priority_level']}, score: {profile['priority_score']})"
        )  # Debug
...
async def update_project(
    project_id: str, project_data: ProjectUpdate, db_conn=Depends(get_database)
):
    ...
    await db_conn.projects.update_one({"id": project_id}, {"$set": update_data})
...
async def upload_music(project_id: str, file: UploadFile = File(...)):
    ...
    # Check disk space
    has_space, message = file_validator.check_disk_space(
        UPLOADS_DIR, app_config.MAX_MUSIC_SIZE
    )
    if not has_space:
...
async def upload_face_image(file: UploadFile = File(...)):
    ...
    # Check disk space
    has_space, message = file_validator.check_disk_space(
        UPLOADS_DIR, app_config.MAX_IMAGE_SIZE
    )
    if not has_space:
...
async def upload_character_images(
    ...
    if face_image:
        ...
        uploaded_files["face_image"] = (
            f"/uploads/characters/{character_dir.name}/{face_filename}"
        )
    ...
    if full_body_image:
        ...
        uploaded_files["full_body_image"] = (
            f"/uploads/characters/{character_dir.name}/{body_filename}"
        )
        ...
        for idx, ref_img in enumerate(reference_images):
            if ref_img:
                ...
                reference_urls.append(
                    f"/uploads/characters/{character_dir.name}/{ref_filename}"
                )
    ...
    for i, prompt_data in enumerate(prompts[:count]):
        try:
            if controlnet_type == "openpose" and character.face_image:
                ...
                and character.reference_images
            ):
...
def get_profile_prompts(
    profile_type: str, pose_style: str, character: Character
) -> List[Dict]:
...
async def get_project_timeline(project_id: str):
    ...
    try:
        ...
        scenes = (
            await db.scenes.find({"project_id": project_id}).sort("order").to_list(100)
        )
        ...
        for scene in scenes:
            ...
            clips = (
                await db.clips.find({"scene_id": scene["id"]})
                .sort("order")
                .to_list(100)
            )
        ...
        return {"project": project_data, "scenes": timeline_scenes}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_project_clips(project_id: str, db_conn=Depends(get_database)):
    clips = (
        await db_conn.clips.find({"project_id": project_id}).sort("order").to_list(100)
    )
...
async def update_scene(scene_id: str, scene_update: SceneUpdate):
    ...
    result = await db.scenes.update_one({"id": scene_id}, {"$set": update_data})
...
async def update_clip(clip_id: str, clip_update: ClipUpdate):
    ...
    update_data = {
        k: v for k, v in clip_update.dict(exclude_unset=True).items() if v is not None
    }
...
async def update_clip_prompts(
    clip_id: str, image_prompt: str = "", video_prompt: str = ""
):
    result = await db.clips.update_one(
        {"id": clip_id},
        {
...
async def select_clip_content(clip_id: str, content_id: str, content_type: str):
    ...
    update_field = (
        "selected_image_id" if content_type == "image" else "selected_video_id"
    )

    result = await db.clips.update_one(
        {"id": clip_id},
        {"$set": {update_field: content_id, "updated_at": datetime.now(timezone.utc)}},
    )
    ...
    if clip_data:
        ...
        await db.clips.update_one({"id": clip_id}, {"$set": {field_name: content_list}})
...
async def get_project_pool(
    project_id: str, content_type: Optional[str] = None, tags: Optional[str] = None
):
    ...
    pool_items = (
        await db.generation_pool.find(query).sort("created_at", -1).to_list(1000)
    )
...
async def update_pool_item(
    ...
    result = await db.generation_pool.update_one({"id": item_id}, {"$set": update_data})
...
async def apply_pool_item_to_clip(item_id: str, clip_id: str):
    ...
    if pool_item.content_type == "image":
        ...
        new_image = {
            ...
            "prompt": pool_item.generation_params.get("prompt", "")
            if pool_item.generation_params
            else "",
            "seed": pool_item.generation_params.get("seed")
            if pool_item.generation_params
            else None,
            "model": pool_item.generation_params.get("model", "unknown")
            if pool_item.generation_params
            else "unknown",
            "is_selected": False,
            "created_at": datetime.now(timezone.utc),
        ...
        result = await db.clips.update_one(
            {"id": clip_id}, {"$push": {"generated_images": new_image}}
        )
        ...
        result = await db.clips.update_one(
            {"id": clip_id}, {"$push": {"generated_videos": new_video}}
        )
    else:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported content type: {pool_item.content_type}",
        )

    return {
        "message": f"Pool item applied to clip successfully",
...
async def get_model_defaults_api(model_name: str):
    ...
    return {"model_name": model_name, "detected_type": model_type, "defaults": defaults}
...
async def get_model_presets(model_name: str, db_conn=Depends(get_database)):
        ...
        if isinstance(preset_config, dict) and preset_name in ["fast", "quality"]:
            presets[preset_name] = {
                ...
                "additional_params": {
                    ...
                    if k
                    not in ["cfg", "steps", "sampler", "scheduler", "width", "height"]
                },
                "is_default": True,
            }
    ...
    if model_data:
        ...
        model_presets = await db_conn.model_presets.find(
...
async def list_style_templates(
    category: Optional[str] = None, public_only: bool = False
):
...
async def update_style_template(template_id: str, template_data: StyleTemplateCreate):
    ...
    result = await db.style_templates.update_one(
        {"id": template_id}, {"$set": update_dict}
    )
...
async def use_style_template(template_id: str):
    ...
    result = await db.style_templates.update_one(
        {"id": template_id}, {"$inc": {"use_count": 1}}
    )
    if result.matched_count == 0:
        raise ResourceNotFoundError("Template", template_id)
...
async def export_final_cut_pro(project_id: str):
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=project_{project_id}.fcpxml"
            },
        )
...
async def export_premiere_edl(project_id: str):
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=project_{project_id}.edl"
            },
        )
    except ValueError as e:
        # Export service raises ValueError for "Project not found"
...
async def export_davinci_resolve(project_id: str):
    ...
    try:
        ...
        return Response(
            ...
            media_type="application/xml",
            headers={
                "Content-Disposition": f"attachment; filename=project_{project_id}_resolve.xml"
            },
        )
    except ValueError as e:
        # Export service raises ValueError for "Project not found"
...
async def create_character(character_data: CharacterCreate):
    ...
    logger.info(
        f"Created character: {character.id} for project: {character.project_id}"
    )
    return character
...
async def update_character(character_id: str, character_data: CharacterCreate):
    ...
    result = await db.characters.update_one({"id": character_id}, {"$set": update_dict})
...
async def generate_character_samples(
    ...
    if character.generation_method == "ip_adapter" and character.reference_images:
        ...
        for i in range(samples):
            ...
            if result_url:
                results.append(
                    {"url": result_url, "prompt": sample_prompt, "method": "ip_adapter"}
                )
        ...
        for i in range(samples):
            ...
            if result_url:
                results.append(
                    {"url": result_url, "prompt": sample_prompt, "method": "reactor"}
                )
        ...
        for i in range(samples):
            ...
            if result_url:
                results.append(
                    {"url": result_url, "prompt": sample_prompt, "method": "instantid"}
                )
        ...
        for i in range(samples):
            ...
            if result_url:
                results.append(
                    {"url": result_url, "prompt": sample_prompt, "method": "standard"}
                )
...
async def enhance_character_face(
    ...
    if not await client.check_connection():
        raise ServiceUnavailableError(
            "FaceFusion", "FaceFusion server is not accessible"
        )
...
async def check_facefusion_status(facefusion_url: str = "http://localhost:7870"):
    ...
    return {
        ...
        if is_online
        else "FaceFusion server is not accessible",
    }
...
async def batch_process_character_faces(
    ...
    for operation in operations:
        ...
        if not image_url:
            results.append(
                {"operation": operation, "success": False, "error": "Missing image_url"}
            )
            continue
        ...
        if image_url.startswith(f"{API}/"):
            ...
            results.append(
                {"operation": operation, "success": False, "error": "Invalid image URL"}
            )
            continue
...
async def register_server_for_queue(
    server_id: str, server_name: str, is_online: bool = True, max_concurrent: int = 1
):
...
# Add CORS middleware with allow-all configuration BEFORE mounting routers
from config import config as app_config

# CORS is intentionally neutralized to never block any request
app.add_middleware(
    CORSMiddleware,
...
async def find_best_civitai_match_mongodb(
    server_model_name: str, db_conn, fallback: bool = False
) -> dict:
    """
    ...
    print(
        f"Searching MongoDB for: '{server_name_clean}' (original: '{server_model_name}')"
    )  # Debug
    ...
    candidates = await db_conn.database_models.find(
        {
            ...
            "$or": [
                {"name": {"$regex": server_name_clean, "$options": "i"}},
                {
                    "civitai_info.description": {
                        "$regex": server_name_clean,
                        "$options": "i",
                    }
                },
    ...
    if best_score < min_threshold:
        print(
            f"No match found above threshold {min_threshold}. Best score was {best_score}"
        )  # Debug
        return None

    if best_match:
        print(
            f"Best match: '{best_match.get('name')}' with score {best_score:.3f}"
        )  # Debug
        return best_match
...
def find_best_civitai_match_local(
    server_model_name: str, civitai_database: list, fallback: bool = False
) -> dict:
    """
    ...
    print(
        f"Searching local database for: '{server_name_clean}' (original: '{server_model_name}')"
    )  # Debug
    ...
    for model in civitai_database:
        ...
        if (
            server_name_clean == model_name_clean
            or server_name_original == model_name_lower
        ):
            score = 1.0
            ...
            for version in model["modelVersions"]:
                if "files" in version:
                    for file_info in version["files"]:
                        ...
                        if (
                            server_name_clean == filename_clean
                            or server_name_original == filename.lower()
                        ):
        ...
        if score < 0.7:
            similarity = SequenceMatcher(
                None, server_name_clean, model_name_clean
            ).ratio()
        ...
        if score < 0.5:
            if any(
                word in model_name_lower
                for word in server_name_clean.split()
                if len(word) > 2
            ):
                score = 0.4
    ...
    if best_match:
        print(
            f"Best match: '{best_match.get('name')}' with score {best_score:.3f} ({match_details[0] if match_details else 'unknown'})"
        )  # Debug
        return best_match
...
async def startup_db_client():
    ...
    if not success:
        logger.critical(
            "Failed to connect to database. Application may not function properly."
        )
        ...
        active_models_service = ActiveModelsService(
            db_manager.client, db_manager.db_name
        )
